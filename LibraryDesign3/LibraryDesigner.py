...
class countTable():
    ...
    def consume(self,sq,verbose=False):
        ...
        if len(sq)>=word:
            ...
            for i in iters:
                self.ints.append(seq2Int(sq_[i:i+word]))
    def get(self,seq,rc=False):
        ...
        if self.fromfile:
            ...
            if self.sparse:
                ...
                if len(ints)>0 and len(self.matrix)>0:    
...
class pb_reports_class:
        ...
        while True:
            ...
            try:
                ...
                if files is not None:
                    ...
                    if self.check_extension(files,fasta_ext):
                    ...
                    elif len(files_)==1 and self.check_extension(files,'bin'):
                        ...
                        print("Extension error or more than 1 bin/pkl file provided.")
                else:
                    print("No files")
    ...
    def compute_pb_report(self,verbose=False):
        ...
        for k in range(len(gen_seqs)):
            ...
            irange = np.arange(len(reg_t)-pb_len)
            if verbose: 
                from tqdm import tqdm
                irange = tqdm(irange)
            for i in irange: #### index in the sequence
                pb_t = reg_t[i:i+pb_len]
                pb_ts = [pb_t,seqrc(pb_t)] if input_use_revc else [pb_t]
                for pb_t in pb_ts:
